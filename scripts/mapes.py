...
async def on_message(server_message):
    ...
    if server_message.content.startswith('mapes') and len(server_message.content.split(' ')) >= 2 and allowed_channel == True:
...
@client.tree.command(name='change_bot_personality', description='Change model')
